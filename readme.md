...
logs.txt
   │
   ▼
utils.py        → log parsing & normalization
   │
   ▼
detect_rules.py → rule-based detections
detect_anomaly.py → anomaly-based detections
   │
   ▼
correlate.py    → signal aggregation & scoring
   │
   ▼
ui.py           → analyst-facing views & visualizations
